...
def find_predictor(user, restaurants, feature_fn):
    ...
    ss_xx=sum([(x-mean(xs))**2 for x in xs]) 
    ss_yy=sum([(y-mean(ys))**2 for y in ys])
    # print (zip(xs,ys))
    ss_xy=sum([(x-mean(xs))*(y-mean(ys)) for x,y in zip(xs,ys)])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** REPLACE THIS LINE ***"
    feature_max_rsq=max(feature_fns, key=lambda x: find_predictor(user,reviewed,x)[1])
    
    return find_predictor(user,reviewed,feature_max_rsq)[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    # END Question 9
"**make variable with restaurant name "
